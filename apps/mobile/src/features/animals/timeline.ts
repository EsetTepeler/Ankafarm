...
import { animals, breedingRecords, groupMovements, groups, healthRecords, lambingRecords, weightRecords } from "@/db/schema";
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<AnimalEvent[]> => {
      ...
      const [row] = await db
        ...
        .limit(1);
      if (!row) return [];
      const [weights, movements, health, breedings, lambings, groupRows] = await Promise.all([
