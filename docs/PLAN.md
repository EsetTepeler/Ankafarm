...
- [x] 0.3 Auth: giriş, refresh (döndürmeli), çıkış, sahip kullanıcı açar (`users.create`), rol middleware'i, seed ile ilk sahip hesabı. (2026-09-20. Refresh token web'de de gövdede döner ve localStorage'da tutulur; HttpOnly cookie sertleştirmesi 4.x'te, çapraz alan adı CORS ve SameSite ayarıyla birlikte.) 2026-09-21: kullanıcı yönetimi arayüzü tamamlandı (Ayarlar > Kullanıcılar): ekleme, ad/rol/telefon düzenleme, şifre sıfırlama ve devre dışı bırakma. `users.update` ve `users.setPassword` uçları eklendi; ikisi de son aktif sahibi koruyor (rolü düşürülemez, kapatılamaz), şifre değişince o kullanıcının refresh token'ları iptal ediliyor. Şifre e-posta ile gönderilmiyor, sahip üretip elden veriyor.
